...
class Lunar(object):
    ...
    def getDays(self):
        ...
        while self.year > constYear:
            if self.isRunNian(constYear):
                days += 1
            days += 365
            constYear += 1
        if self.month > 1:
            days += p[self.month-2]
            if self.isRunNian(self.year): #闰年且月份大于1
                days += 1
        days += self.day
        return days-1
...
# 测试
def test(year = 1990,month = 1,day = 1):
    ln = Lunar(year,month,day)    
    print('公历 {}-{}-{}'.format(year,month,day))
    print('{} 【{}】 {}年 {}日'.format(ln.ln_date_str(), ln.gz_year(), ln.sx_year(), ln.gz_day()))
    print('节气：{}'.format(ln.ln_jie()))
    str = '{} 【{}】 {}年 {}日'.format(ln.ln_date_str(), ln.gz_year(), ln.sx_year(), ln.gz_day())
    str += '节气：{}'.format(ln.ln_jie())
    return '{} 【{}】 {}年 {}日'.format(ln.ln_date_str(), ln.gz_year(), ln.sx_year(), ln.gz_day())+'节气：{}'.format(ln.ln_jie())
